    ...
    def annotate(self, doc):
        ...
        for span in doc.tiers['structured_data'].spans:
            ...
            for column_values in table_by_column:
                ...
                # TODO key, value pairs cannot always be treated as columns since
                ...
                for value_type, value_spans in entities_by_type.items():
                    column_entities = [
                        ...
                        for group_span, contained_spans in column_values.group_spans_by_containing_span(value_spans)
                    ]
                    num_matches = sum([contained_spans is not None for contained_spans in column_entities])
                    if float(num_matches) / total_rows > 0.3:
        ...
        for table in tables:
            for row_idx, row in enumerate(table.rows):
                ...
                # If a count is marked as incremental, any count in the row above that value is considered cumulative.
                max_new_cases = 0
                max_new_deaths = 0
                ...
                print max_new_cases, max_new_deaths
                for incident_span in row_incidents:
                    incident = incident_span.metadata
                    if incident['aggregation'] == None:
                        if incident['base_type'] == 'caseCount':
                            if incident['value'] > max_new_cases:
                                incident['aggregation'] = 'cumulative'
                        else:
                            if incident['value'] > max_new_deaths:
                                incident['aggregation'] = 'cumulative'
                for incident_span in row_incidents:
                    incident = incident_span.metadata
